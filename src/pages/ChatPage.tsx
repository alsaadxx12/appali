    ...
    if (activeChat) {
        ...
        return (
            <div className="page-content page-enter chat-root" style={{ display: 'flex', flexDirection: 'column', height: 'calc(100dvh - 60px - 70px)', padding: 0, overflow: 'hidden', position: 'relative', width: '100%', maxWidth: '100vw', marginBottom: 0 }}>
                ...
                {/* Input bar */}
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 8px', borderTop: '1px solid var(--border-glass)', background: 'var(--bg-card)', flexShrink: 0 }}>
                    <button onClick={editMsg ? doEdit : () => sendMessage()} disabled={editMsg ? !editTxt.trim() : (!newMsg.trim() || sending)} style={{ width: 36, height: 36, borderRadius: '50%', background: (editMsg ? editTxt.trim() : newMsg.trim()) ? 'linear-gradient(135deg,#3b82f6,#6366f1)' : 'var(--bg-glass)', border: 'none', color: (editMsg ? editTxt.trim() : newMsg.trim()) ? 'white' : 'var(--text-muted)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: (editMsg ? editTxt.trim() : newMsg.trim()) ? 'pointer' : 'default', transition: 'all .2s', flexShrink: 0, boxShadow: (editMsg ? editTxt.trim() : newMsg.trim()) ? '0 4px 16px rgba(59,130,246,.35)' : 'none' }}>{editMsg ? <Check size={16} /> : <Send size={16} style={{ transform: 'rotate(180deg)' }} />}</button>
                    <div style={{ flex: 1, display: 'flex', alignItems: 'center', background: 'var(--bg-glass)', border: editMsg ? '1px solid rgba(59,130,246,.3)' : '1px solid var(--border-glass)', borderRadius: 20, padding: '0 4px 0 4px', minHeight: 36, transition: 'all .2s' }}>
