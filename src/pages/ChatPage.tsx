...
export default function ChatPage({ onBack }: Props) {
    ...
    if (activeChat) {
        ...
        return (
            <div className="page-content page-enter chat-root" style={{ display: 'flex', flexDirection: 'column', height: 'calc(100dvh - 60px - 70px)', padding: 0, overflow: 'hidden', position: 'relative', width: '100%', maxWidth: '100vw', marginBottom: 0 }}>
                <style>{css}</style>
                {/* Top header */}
                ...
                {/* Input bar */}
                <div style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '5px 6px', borderTop: '1px solid var(--border-glass)', background: 'var(--bg-card)', flexShrink: 0, maxWidth: '100%', overflow: 'hidden', boxSizing: 'border-box' }}>
                    <button onClick={editMsg ? doEdit : () => sendMessage()} disabled={editMsg ? !editTxt.trim() : (!newMsg.trim() || sending)} style={{ width: 34, height: 34, borderRadius: '50%', background: (editMsg ? editTxt.trim() : newMsg.trim()) ? 'linear-gradient(135deg,#3b82f6,#6366f1)' : 'var(--bg-glass)', border: 'none', color: (editMsg ? editTxt.trim() : newMsg.trim()) ? 'white' : 'var(--text-muted)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: (editMsg ? editTxt.trim() : newMsg.trim()) ? 'pointer' : 'default', transition: 'all .2s', flexShrink: 0 }}>{editMsg ? <Check size={15} /> : <Send size={15} style={{ transform: 'rotate(180deg)' }} />}</button>
                    <div style={{ flex: 1, display: 'flex', alignItems: 'center', background: 'var(--bg-glass)', border: editMsg ? '1px solid rgba(59,130,246,.3)' : '1px solid var(--border-glass)', borderRadius: 18, padding: '0 2px 0 4px', minHeight: 34, overflow: 'hidden', minWidth: 0 }}>
                        {editMsg ? <input type="text" maxLength={100} value={editTxt} onChange={e => setEditTxt(e.target.value)} onKeyDown={e => e.key === 'Enter' && doEdit()} autoFocus style={{ flex: 1, padding: '6px 4px', background: 'transparent', border: 'none', color: 'var(--text-primary)', fontSize: 13, fontWeight: 700, fontFamily: 'var(--font-arabic)', outline: 'none', minWidth: 0, width: 0 }} /> :
                            <input ref={inpRef} type="text" maxLength={100} placeholder="اكتب رسالة..." value={newMsg} onChange={e => setNewMsg(e.target.value)} onKeyDown={e => e.key === 'Enter' && !e.shiftKey && sendMessage()} style={{ flex: 1, padding: '6px 4px', background: 'transparent', border: 'none', color: 'var(--text-primary)', fontSize: 13, fontWeight: 700, fontFamily: 'var(--font-arabic)', outline: 'none', minWidth: 0, width: 0 }} />}
                        <button onClick={() => { setShowEmoji(!showEmoji); setShowAttach(false); }} style={{ width: 28, height: 28, borderRadius: '50%', background: 'transparent', border: 'none', cursor: 'pointer', color: showEmoji ? '#f59e0b' : 'var(--text-muted)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, padding: 0 }}><Smile size={17} /></button>
                        <div style={{ position: 'relative', flexShrink: 0 }}>
                            <button onClick={() => { setShowAttach(!showAttach); setShowEmoji(false); }} style={{ width: 28, height: 28, borderRadius: '50%', background: 'transparent', border: 'none', cursor: 'pointer', color: showAttach ? '#3b82f6' : 'var(--text-muted)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 0 }}><Paperclip size={17} /></button>
                            {showAttach && <div style={{ position: 'absolute', bottom: 36, left: 0, background: 'var(--bg-card)', border: '1px solid var(--border-glass)', borderRadius: 'var(--radius-lg)', padding: 6, boxShadow: '0 8px 32px rgba(0,0,0,.3)', animation: 'fadeUp .15s ease', display: 'flex', flexDirection: 'column', gap: 2, minWidth: 150, zIndex: 50 }}>
                                ...
                                <button className="ci" onClick={sendLocation}><MapPin size={14} style={{ color: '#f59e0b' }} />📍 الموقع</button>
                            </div>}
                        </div>
                    </div>
                </div>
