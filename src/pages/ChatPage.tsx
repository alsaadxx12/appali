    ...
    if (activeChat) {
        ...
        return (
            <div className="page-content page-enter" style={{ display: 'flex', flexDirection: 'column', height: 'calc(100dvh - var(--nav-height) - 60px)', padding: 0 }}>
                ...
                {/* Input bar */}
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '10px 12px', borderTop: '1px solid var(--border-glass)', background: 'var(--bg-card)', flexShrink: 0 }}>
                    <button onClick={() => { setShowEmoji(!showEmoji); setShowAttach(false); }} style={{ width: 38, height: 38, borderRadius: '50%', background: showEmoji ? 'rgba(245,158,11,.12)' : 'transparent', border: 'none', cursor: 'pointer', color: showEmoji ? '#f59e0b' : 'var(--text-muted)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><Smile size={22} /></button>
                    <div style={{ position: 'relative', flexShrink: 0 }}>
                        <button onClick={() => { setShowAttach(!showAttach); setShowEmoji(false); }} style={{ width: 38, height: 38, borderRadius: '50%', background: showAttach ? 'rgba(59,130,246,.12)' : 'transparent', border: 'none', cursor: 'pointer', color: showAttach ? '#3b82f6' : 'var(--text-muted)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Paperclip size={22} /></button>
                        {showAttach && <div style={{ position: 'absolute', bottom: 44, right: 0, background: 'var(--bg-card)', border: '1px solid var(--border-glass)', borderRadius: 'var(--radius-lg)', padding: 6, boxShadow: '0 8px 32px rgba(0,0,0,.3)', animation: 'fadeUp .15s ease', display: 'flex', flexDirection: 'column', gap: 2, minWidth: 160 }}>
                    ...
                    {editMsg ? <input type="text" value={editTxt} onChange={e => setEditTxt(e.target.value)} onKeyDown={e => e.key === 'Enter' && doEdit()} autoFocus style={{ flex: 1, padding: '10px 16px', borderRadius: 'var(--radius-full)', background: 'var(--bg-glass)', border: '1px solid rgba(59,130,246,.3)', color: 'var(--text-primary)', fontSize: 14, fontWeight: 700, fontFamily: 'var(--font-arabic)', outline: 'none' }} /> :
                        <input ref={inpRef} type="text" placeholder="اكتب رسالة..." value={newMsg} onChange={e => setNewMsg(e.target.value)} onKeyDown={e => e.key === 'Enter' && !e.shiftKey && sendMessage()} style={{ flex: 1, padding: '10px 16px', borderRadius: 'var(--radius-full)', background: 'var(--bg-glass)', border: '1px solid var(--border-glass)', color: 'var(--text-primary)', fontSize: 14, fontWeight: 700, fontFamily: 'var(--font-arabic)', outline: 'none' }} />}
                    <button onClick={editMsg ? doEdit : () => sendMessage()} disabled={editMsg ? !editTxt.trim() : (!newMsg.trim() || sending)} style={{ width: 42, height: 42, borderRadius: '50%', background: (editMsg ? editTxt.trim() : newMsg.trim()) ? 'linear-gradient(135deg,#3b82f6,#6366f1)' : 'var(--bg-glass)', border: 'none', color: (editMsg ? editTxt.trim() : newMsg.trim()) ? 'white' : 'var(--text-muted)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: (editMsg ? editTxt.trim() : newMsg.trim()) ? 'pointer' : 'default', transition: 'all .2s', flexShrink: 0, boxShadow: (editMsg ? editTxt.trim() : newMsg.trim()) ? '0 4px 16px rgba(59,130,246,.35)' : 'none' }}>{editMsg ? <Check size={18} /> : <Send size={18} style={{ transform: 'rotate(180deg)' }} />}</button>
                </div>
