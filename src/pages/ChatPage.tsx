...
export default function ChatPage({ onBack, onChatActive }: Props) {
    ...
    const sendContact = async () => { setShowAttach(false); try { if ('contacts' in navigator && 'ContactsManager' in window) { const props = ['name', 'tel']; const opts = { multiple: false }; const contacts = await (navigator as any).contacts.select(props, opts); if (contacts?.length) { const c = contacts[0]; const name = c.name?.[0] || 'جهة اتصال'; const phones = (c.tel || []).filter(Boolean); const tel = phones.join(' / ') || 'بدون رقم'; await sendMessage({ type: 'text', text: `👤 ${name}\n📞 ${tel}` }); } } else { alert('هذه الميزة تتطلب فتح التطبيق من متصفح Chrome على Android.\nلا يدعم iOS Safari الوصول لجهات الاتصال حالياً.'); } } catch (e) { if ((e as any)?.name !== 'TypeError') console.error(e); } };
